import type { FoundersList, IVRTwitchUser, ModVIPList } from "./Models/IVR";
...
export async function GetFounders(channel: string): Promise<FoundersList | null> {
    const API_URL = import.meta.env.VITE_IVR_API_URL;
    if (!API_URL) throw new Error("IVR_API_URL is not defined in environment variables.");

    const response = await fetch(`${API_URL}/v2/twitch/founders/${channel.toLowerCase()}?skipCache=true`);
    if (!response.ok) {
        console.log(`API request failed: ${response.statusText}`);
        return null;
    }
