...
export async function PostStreamElementsCode(
	userId: string,
	newCode: Partial<TurtleCode>
): Promise<boolean> {
	const API_URL = import.meta.env.VITE_API_URL;
	if (!API_URL) throw new Error('API_URL is not defined in environment variables.');
...
export async function PostStreamElementsCodesBulk(
	userId: string,
	codesData: Partial<TurtleCode>,
	amount: number
): Promise<TurtleCode[]> {
	const API_URL = import.meta.env.VITE_API_URL;
	if (!API_URL) throw new Error('API_URL is not defined in environment variables.');
...
export async function PutStreamElementsCode(
	userId: string,
	codeId: string,
	updatedData: Partial<TurtleCode>
): Promise<boolean> {
	const API_URL = import.meta.env.VITE_API_URL;
	if (!API_URL) throw new Error('API_URL is not defined in environment variables.');
